...
exports.getRequestData = async function(req, type){
    ...
    const runsWithRequestById = _.indexBy(runsWithRequest, 'id');
    const remoteRunData = (
        await async.mapLimit(runsWithRequest, 5, async (run) => {return req.intercode.getRun(run.event_id, run.id)})
    ).filter((remoteRun) => remoteRun != null);
    return await async.map(remoteRunData, async (remoteRun) => {
        const runRequest = runsWithRequestById[remoteRun.id];
        return {
            ...remoteRun,
...
exports.getRoomData = async function(req, roomId = undefined) {
    const { rooms, events, furniture } = await async.parallel({
        rooms: async () => await req.intercode.getRooms(),
