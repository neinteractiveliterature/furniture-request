    ...
    req.models.runs.list(function(err, runs){
        ...
        async.waterfall([
            function(cb){
                ...
                async.filter(filteredRuns, async function(runRequest, cb){
                    try {
                        const run = await req.intercode.getRun(runRequest.event_id, runRequest.id);
                        if (run) {
                            cb(null, true);
                        } else {
                            cb(null, false);
            ...
            function(filteredRuns, cb){
                async.map(filteredRuns, async function(runRequest, cb){
                    try {
                        const run = await req.intercode.getRun(runRequest.event_id, runRequest.id);
