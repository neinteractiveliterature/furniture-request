...
async function getRunList(events) {
    ...
    const { dbRuns, dbRequests } = await async.parallel({
        dbRuns: async () => await models.runs.getMultiple(runsWithEvent.map((run) => run.id)),
        dbRequests: async () => await models.requests.getMultipleByRunId(runsWithEvent.map((run) => run.id)),
    });
...
async function getFurnitureTree(parentId = null){
    ...
    for (const item of items){
        item.children = await getFurnitureTree(item.id);
        furniture.push(item);
    }
    return furniture;
}
