...
function check(req, permissions, permission){
    ...
    } else if (permission === 'event' && eventId && _.pluck(user.events, 'id').indexOf(Number(eventId)) !== -1){
        return true;
    // Check for event + GM Liaison
    } else if (permission === 'event' && eventId && permissions.indexOf('gm_liaison') !== -1){
        return true;
    // Fail
    } else {
        return false;
