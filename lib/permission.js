...
function check(req, permissions, permission){
    ...
    } else if (permissions.privileges.indexOf('staff') !== -1 || permissions.privileges.indexOf('site_admin') !== -1){
        return true;
    // Conchair and Webteam get all permissions
    } else if (permissions.positions.indexOf('Con Chair') !== -1 || permissions.privileges.indexOf('Website Team') !== -1){
        return true;
    // Check for specific permission in privileges
    } else if (permissions.privileges.indexOf(permission) !== -1){
        return true;
    // Check for specific permission in positions
    } else if (permissions.positions.indexOf(permission) !== -1){
        return true;
    // Check for team member of event
    } else if (eventId && _.pluck(user.events, 'id').indexOf(eventId) !== -1){
    // Fail
    } else {
        return false;
    }
